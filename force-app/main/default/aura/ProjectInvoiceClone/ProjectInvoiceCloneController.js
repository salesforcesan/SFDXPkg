({
    doInit : function(component, event, helper) {        
        
        helper.loadProjectInvoiceCloneReasons(component);
        helper.getProjectInvoiceDetails(component);
    },
    cloneButtonClicked : function(component, event, helper) { 
        
        //Validation
        var inputCmp = component.find("projectInvoiceCloneReasonCode");
        var value = inputCmp.get("v.value");        
        var commentCmp = component.find("cloneReasonComment");		
        var commentTxt = commentCmp.get("v.value");
        
        inputCmp.set("v.errors", null);
        commentCmp.set("v.errors",null);
        
        if (value==null || value=='--SELECT--') {            
            inputCmp.set("v.errors", [{message:"Select clone reason"}]);
        }else if (value=='Other' && (commentTxt==null || commentTxt.trim()=='')) {        
            commentCmp.set("v.errors", [{message:"Please enter comments"}]);
        }
            else {    
                helper.CloneProjectInvoice(component, event, helper);
            }
    },
    cancelButtonClicked : function(component, event, helper) { 
        helper.CancelClicked(component, event, helper);
    },
    
})