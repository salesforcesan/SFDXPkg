  ...
  A.module("cmk.service.lookupModel")
    .factory('cmkServiceLookupModel', ["$q", function($q) {
      ...
      function genLocations(count, keyword) {
        ...
        if (!!keyword) {
          data = data.filter(function(l) {
            return (l.LocationNumber__c + "").indexOf(keyword) !== -1 ||
              l.Address__c.toLowerCase().indexOf(keyword) !== -1 ||
              l.City__c.toLowerCase().indexOf(keyword) !== -1 ||
              l.State__c.toLowerCase().indexOf(keyword) !== -1;
          });
        }
      ...
      FakeModel.prototype.findLocations = function(keyword) {
        return genLocations(50, keyword);
      };
      ...
      FakeModel.prototype.findProjects = function(query) {
        ...
        data.forEach(function(rec) {
          var startDate = getDate(i % 2 === 0 ? i % 300 : -(i % 300)),
            endDate = moment(startDate).add(i % 10, "days");
          ...
          rec.StartDate__c = startDate.toISOString();
          ...
          rec.EndDate__c = endDate.toISOString();
          i++;
        });
