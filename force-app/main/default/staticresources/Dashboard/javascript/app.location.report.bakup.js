(function(A) {
      ...
      _model.findLocations(keyword).then(function(data) {
        if (!!data) {
          data.forEach(function(rec) {
            rec.Num = "<span class='label label--success'>" + rec.Num + "</span>";
          });
        }
        _self.locationModel.rowset = data;
  ...
  function transformEvents(viewType, rowset) {
    ...
    rowset.forEach(function(rec) {
      startDate = moment(Date.parse(rec.StartDate__c));
      endDate = moment(Date.parse(rec.EndDate__c));
      events.push({
      ...
      for (var i = 0; i < diffDays; i++) {
        if (eventCounts[startDate.add(i, 'days').unix()] == null) {
          eventCounts[startDate.add(i, 'days').unix()] = 1;
        } else {
          eventCounts[startDate.add(i, 'days').unix()] += 1;
        }
      }
