(function(A){
    ...
    A.module("app.project", [
     ...
     .controller("ProjectController", ["$scope", "cmkWebContext", "$timeout", function($scope, webContext, $timeout){
        ...
        $scope.dialogModel = {
          ...
          isLargeModal: 1,
          buttons: [
            {label: "Cancel", value: "Cancel 123", isPrimary: 0, onClick: function(event){alert(event.target.value);}},
            {label: "Submit", value: "Submit 123", isPrimary: 1, onClick: function(event){alert(event.target.value);}}
          ]
        };
        $timeout(function(){
          $scope.dialogModel.showDialog = true;
        }, 1000);
     }]);
