     ...
     return {
       ...
       template: function(tElement, tAttrs) {
         return '<article class="cm-article">' +
           ...
           '<div class="slds-float--left"><h2 class="slds-text-heading--label slds-text-heading--large cm-article-title" ng-bind-html="uiModel.title"></h2></div>' +
           '<div class="slds-float--right"><div class="cm-widget-hdr-group" ng-bind-html="uiModel.toolbarHTML"></div></div>' +
           '</header>' +
   ...
   .controller("PageletController", ["$scope", "$log", "cmkWebContext", function($scope, $log, webContext) {
     ...
     function buttonHtml(bar) {
       return ['<a href="#',
         ...
         '" title="',
         bar.text,
         '" class="slds-button slds-button--icon-border-filled cm-widget-hdr-button">',
         '<svg aria-hidden="true" class="slds-button__icon slds-button__icon--small cm-icon">',
         '<use xlink:href="',
         bar.iconUrl,
         '"></use></svg><span class="slds-assistive-text">',
         bar.text,
         '</span></a>'
       ].join("");
     }
