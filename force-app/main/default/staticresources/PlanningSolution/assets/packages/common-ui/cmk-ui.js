      ...
      return {
        ...
        template: function() {
          return '<div aria-expanded="{{uiModel.isHidden === false}}" class="slds-picklist">' +
            '<button class="slds-button slds-button--neutral slds-picklist__label {{cssclass}} slds-truncate" ng-blur="onBlur()" data-id="{{model.option.key}}" ng-click="wantToSelect($event)" aria-haspopup="true">' +
            ...
            '<div class="slds-dropdown slds-dropdown--small slds-dropdown--menu {{uiModel.alignment}} cm-dropdown-menu" ng-hide="uiModel.isHidden">' +
            '<ul class="slds-dropdown__list" role="menu" ng-style="uiModel.lookupPostionStyle" ng-mouseover="onMouseOver()" ng-mouseleave="onMouseLeave()">' +
            '<li ng-repeat="item in model.options" class="slds-dropdown__item slds-has-icon--left" aria-selected="{{item.selected}}" tabindex="-1" role="option">' +
            '<a href="#" data-id="{{$index}}" class="slds-truncate">' +
            '<svg aria-hidden="true" ng-if="item.selected" class="slds-icon slds-icon--small slds-icon--left"><use xlink:href="' + _staticPath + '/assets/icons/standard-sprite/svg/symbols.svg#task2"></use></svg>' +
     ...
     A.module("cmk.ui.pill", ["cmk.web.context", "ngSanitize"])
       .directive("cmkPill", ["cmkWebContext", "$log", function(webContext, $log) {
         ...
         return {
           ...
           template: function() {
             return '<span ng-if="isHidden == 0"><span class="slds-pill"><span class="slds-pill__label" ng-bind-html="title"></span><button class="slds-button slds-button--icon-bare">' +
               '<svg aria-hidden="true" class="slds-button__icon"><use xlink:href="' + _staticPath + '/assets/icons/utility-sprite/svg/symbols.svg#close"></use>' +
               '</svg><span class="slds-assistive-text">Remove</span></button></span></span>';
           },
